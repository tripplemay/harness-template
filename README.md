...
**UI 工程化（设计系统先行）**
- 新项目第一个批次必须是设计系统（颜色 token、排版、基础组件、公共 hook、布局框架），不是第一个业务页面。用一个试点页面验证设计系统，验证通过后再展开其他页面
- 有原型 ≠ 有设计系统。原型解决"页面长什么样"，设计系统解决"所有页面怎么一致地长成这样"。跳过设计系统直接复刻原型，每个页面都会变成独立的自包含代码，后期统一改动成本是页面数量的倍数
- 给 AI agent 的 spec 必须指定"用什么组件实现"，不能只说"做成什么样"。AI 天然倾向生成自包含代码，不会主动抽取公共组件
- 第一个页面完成后做一次组件审计：有无硬编码颜色、内联样式、重复逻辑。有则先抽组件再做第二个页面，不要让重复代码扩散
- 来源：AIGC Gateway P1 按原型逐页实现后，R1~R2 又花了 3 个批次（28 个功能）重构所有页面对齐设计系统，重复劳动本可避免
