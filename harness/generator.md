...
2. **提取结构清单**：从原型中列出页面骨架（区块划分）、表格列定义、表单字段、交互入口（按钮/开关/下拉）、数据绑定路径
3. **识别新增功能**：对比原型与现有代码，标记原型中有但现有代码中没有的功能（如编辑态、新列、新操作），这些是需要新增实现的，不是纯视觉调整
4. **逐块实现**：按结构清单逐块编码，每完成一个区块回头对照原型确认
5. **完成后 1:1 核对**：再次 Read 原型 HTML，逐项确认实现与原型的 DOM 结构、字段、交互一致
